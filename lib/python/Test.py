...
def init():
	...
	# make XML files
	for i in range(NUM_PORTS):
		filename = expectPHY_fileHeader + str(i+1) #create a file for each port
		f_expectPHY.append(open(directory+'/'+filename, 'w'))
		writeXMLHeader(fExpectPHY(i+1), directory+"/"+filename)

	for i in range(NUM_PORTS):
		filename = expectDMA_fileHeader + str(i+1) #create a file for each port
...
def writeXMLHeader(fp, filePath):
	...
	if str.find(filePath, expectPHY_fileHeader)>0:
		fp.write("<!-- PHYS_PORTS = "+str(NUM_PORTS)+" MAX_PORTS = "+str(NF2_MAX_PORTS)+" -->\n")
