...
# Objective:
# generate IP packets, with protocols other than OSPF/ICMP/TCP/UDP
# and make sure they are dropped
...
if ( !( $pid = fork ) ) {
	...
	try {
		...
		# Send packets with invalid protos and expect proto unreach messages
		for (my $i = 0; $i < 256; $i++) {
			if ( ( $i != 1 ) && ( $i != 6 ) && ( $i != 17 ) && ( $i != 89 ) ) {
				# Send the packet
				my $pkt = nftest_send_IP('192.168.0.100', '192.168.0.2', len => 100, proto => $i);

				# Expect ICMP protocol unreachable message
				nftest_expect_ICMP_proto_unreach('192.168.0.100', '192.168.0.2', $pkt);
			}
		}
