...
# Objective: UPD packet returns port unreachable
# generate a UDP packet, and an ICMP port unreachable packet,
# ensure that software generates the same ICMP packet
...
if ( !( $pid = fork ) ) {
	...
	try {
		...
		# Send a packet
		my $pkt = nftest_send_UDP('192.168.0.100', '192.168.0.2',
			len => 100,
			src_port => 0x0001,
			dst_port => 0xdddd,
		);
		...
		# Expect ICMP port unreachable message
		nftest_expect_ICMP_port_unreach('192.168.0.100', '192.168.0.2', $pkt);
