...
print "Shortcut words (72bit): $shortcut_words\n";
print "DRAM write words (144bit): $dram_wr_words\n";
#DRAM read words == DRAM write words
nftest_regread('nf2c0', DRAM_QUEUE_DRAM_RD_WORDS_REG(), $dram_wr_words);
#Input words == DRAM write words * 2 + shortcut words
#DRAM width is 144 bit, shortcut width is 72 bit
nftest_regread('nf2c0', DRAM_QUEUE_INPUT_WORDS_REG(), $dram_wr_words * 2 + $shortcut_words);
#Output words == input words
nftest_regread('nf2c0', DRAM_QUEUE_OUTPUT_WORDS_REG(), $dram_wr_words * 2 + $shortcut_words);
