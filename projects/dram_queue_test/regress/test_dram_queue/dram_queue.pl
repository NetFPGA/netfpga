...
nftest_regwrite('nf2c0', DRAM_QUEUE_SHORTCUT_DISABLE_REG(), 1);
print nftest_regread('nf2c0', DRAM_QUEUE_SHORTCUT_DISABLE_REG())."\n";
...
nftest_regwrite('nf2c0', DRAM_QUEUE_SHORTCUT_DISABLE_REG(), 0);
`usleep 500`;
my $shortcut_words = nftest_regread('nf2c0', DRAM_QUEUE_SHORTCUT_WORDS_REG());
my $dram_wr_words = nftest_regread('nf2c0', DRAM_QUEUE_DRAM_WR_WORDS_REG());
print "Shortcut words (144bit): $shortcut_words\n";
print "DRAM write words (144bit): $dram_wr_words\n";
#DRAM read words == DRAM write words
nftest_regread_expect('nf2c0', DRAM_QUEUE_DRAM_RD_WORDS_REG(), $dram_wr_words);
#Input words == DRAM write words * 2 + shortcut words * 2
#DRAM width is 144 bit, shortcut width is 144 bit
nftest_regread_expect('nf2c0', DRAM_QUEUE_INPUT_WORDS_REG(), $dram_wr_words * 2 + $shortcut_words * 2);
#Output words == input words
nftest_regread_expect('nf2c0', DRAM_QUEUE_OUTPUT_WORDS_REG(), $dram_wr_words * 2 + $shortcut_words * 2);
