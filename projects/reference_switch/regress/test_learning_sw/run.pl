...
# loop for $num_normal packets from eth2 to eth1
for (my $i = 0; $i < $num_normal; $i++)
{
	# set parameters
	my $DA = "aa:bb:cc:dd:ee:ff";
	my $SA = $routerMAC1;
	my $TTL = 64;
...
$temp_error_val = nftest_regread_expect('nf2c0', MAC_GRP_0_TX_QUEUE_NUM_PKTS_SENT_REG(), $num_normal);

if ($temp_error_val != $num_normal) {
	print "ERROR: Num pkts sent from MAC 0 Tx Queue is $temp_error_val not $num_normal\n";
	$total_errors++;
}
...
$temp_error_val = nftest_regread_expect('nf2c0', SWITCH_OP_LUT_NUM_HITS_REG(), $num_normal);

if ($temp_error_val != $num_normal) {
  print "ERROR: Num Switch LUT hits is $temp_error_val not $num_normal\n";
  $total_errors++;
}
